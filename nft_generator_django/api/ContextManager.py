...
class ContextManager:
    ...
    def new_context(self, context_id: str, jsonstr: str) -> Context:
        ...
        if len(self.contexts) == 1 and self.contexts[context_id].executing == True:
            raise OverflowError("There is a executing process.")
        _new = Context(jsonstr)
        self.contexts[context_id] = _new
        return _new

    def new_and_start_context(self, context_id: str, jsonstr: str, enable_render: bool, enable_metadata: bool):
        _new = self.new_context(context_id, jsonstr)
        _new.run(enable_render, enable_metadata)
